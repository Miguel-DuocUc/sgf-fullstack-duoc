...
CREATE TABLE IDENTITY_DOCUMENTS (
                                    ...
                                    minor NUMBER(1,0) NOT NULL,
                                    notarized_authorization NUMBER(1,0) NOT NULL,
                                    status VARCHAR2(30 CHAR) NOT NULL,
                                    created_at TIMESTAMP NOT NULL,
                                    CONSTRAINT pk_identity_documents PRIMARY KEY (id),
                                    CONSTRAINT uk_identity_document_number UNIQUE (document_number),
                                    CONSTRAINT chk_identity_minor CHECK (minor IN (0,1)),
                                    CONSTRAINT chk_identity_notarized CHECK (notarized_authorization IN (0,1))
);